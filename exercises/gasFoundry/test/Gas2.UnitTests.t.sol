...
contract Gas2Test is Test {
    GasContract2 public gas;
    ...
    function setUp() public {
        vm.startPrank(owner);
        gas = new GasContract2(admins, totalSupply);
        vm.stopPrank();
    }
    ...
    function test_whitelistTransfer(
        ...
        vm.assume(_amount > 3);
        vm.assume(bytes4(_name).length < 9 );
        _tier = bound( _tier, 1, 244);
        vm.startPrank(owner);
    ...
    function testWhiteTranferAmountUpdate(
        ...
        _tier = bound( _tier, 1, 244);
        vm.assume(_amount > 3);
        vm.assume(bytes4(_name).length < 9 && bytes4(_name).length >0);
        vm.startPrank(owner);
        gas.transfer(_sender, _amount, _name);
